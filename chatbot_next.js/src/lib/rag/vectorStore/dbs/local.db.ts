...
// في Vercel لا يمكن الكتابة إلا في /tmp
// محلياً نستخدم المسار من env أو المجلد الافتراضي
const DB_DIR = path.resolve(/*turbopackIgnore: true*/ process.env.LOCAL_DB_PATH ?? "./.local_vector_db");
...
export class VectorStoreLocal {
  ...
  static async upsert(params: { id: string; question: string; answer: string; embedding: number[] }): Promise<void> {
    const index = await readIndex();
  ...
  static async search(embedding: number[], topK: number = 5): Promise<Array<{ id: string; question: string; answer: string; score: number }>> {
    const index = await readIndex();
