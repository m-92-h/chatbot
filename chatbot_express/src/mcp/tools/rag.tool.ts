  ...
  server.registerTool(
    ...
    async ({ query, topK }) => {
      ...
      try {
        ...
        return {
          // النص الصريح الموجه لـ Gemini: يقرأه الموديل كـ Prompt معزز بالسياق لبناء الرد النهائي
          content: [
            {
              type: "text" as const,
              text: result.prompt,
          ...
          // البيانات الهيكلية الخام الموجهة للنظام والواجهة الأمامية
          structuredContent: {
        ...
        // نُعيد رسالة خطأ بدل throw حتى يتمكن Gemini من الاستمرار
        // بالإجابة من معرفته العامة
        return {
          content: [
            {
              type: "text" as const,
